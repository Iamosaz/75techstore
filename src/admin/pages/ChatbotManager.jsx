    ...
    const fetchData = async () => {
      try {
        ...
        const [configRes, analyticsRes] = await Promise.all([
          axios.get(`${API_URL}/chatbot/config`, getAuthConfig()),
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Powered by Groq AI (Llama 3) - Free & Fast</p>
        </div>
        <button onClick={handleSave} disabled={saving}
          className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl
                     font-semibold text-sm transition flex items-center gap-2 disabled:opacity-60">
          {saving ? (
            <>
      ...
      {success && (
        <div className="bg-green-50 border border-green-200 text-green-700
                        px-4 py-3 rounded-xl mb-4 text-sm flex items-center gap-2">
          ✅ {success}
        </div>
      )}
      {error && (
      ...
      {analytics && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'Bot Status',     value: config?.isActive ? 'Online' : 'Offline',
              icon: config?.isActive ? '🟢' : '🔴',
              color: config?.isActive ? 'text-green-600' : 'text-red-600' }
          ].map((stat, i) => (
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200
        ...
        <span className={`ml-auto px-3 py-1 rounded-full text-xs font-bold ${
          config?.isAIEnabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
        }`}>
          ...
          {activeTab === 'general' && config && (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between bg-purple-50 border border-purple-100 p-4 rounded-xl">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={config.isAIEnabled}
          ...
          {activeTab === 'security' && config && (
            <div className="space-y-5">
              <div className="flex items-center justify-between bg-red-50 border border-red-100 p-4 rounded-xl">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked={config.securityEnabled}
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={blogCategory} onChange={(e) => setBlogCategory(e.target.value)}
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm
                               focus:outline-none focus:ring-2 focus:ring-blue-500">
                    {['Tech News','Product Reviews','How To','Deals & Offers',
                      'Gaming','Phones','Laptops','Accessories','Other'].map(cat => (
                ...
                <div>
                  ...
                  <input type="text" value={blogKeywords}
              ...
              <button onClick={handleGenerateBlog} disabled={generating || !blogTopic}
                className="w-full bg-gradient-to-r from-blue-600 to-purple-600
                           hover:from-blue-700 hover:to-purple-700 text-white py-3
                ...
                <div className="bg-white border border-green-200 rounded-xl p-5 space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-600">Saved as draft · Redirecting you now...</p>
                    </div>
                  </div>
